	...
	count=1
          for Ly in Ly_list
   		for i in 1:length(m_list)
	     		 ...
	     		 map[count,2]= round(m_list[i],digits=3) # corresponding m
	      	         map[count,3]= round(W_list[i],digits=3) # corresponding W
...
Ly_list=[28,40,56,80,113]
system_scale =100000

njobs= length(m_list)*length(Ly_list) #number of jobs
